...
// 保存采用“防抖 + 单通道”队列：频繁拖动时只保存最新快照，且不会让旧请求晚到后覆盖新数据。
const SAVE_DEBOUNCE_MS = 400;
let pendingSaveBody = null;
let saveTimer = null;
let saveInFlight = false;

function queueCanvasSave(snapshot, delay = SAVE_DEBOUNCE_MS) {
  pendingSaveBody = JSON.stringify(snapshot);
  if (saveTimer) clearTimeout(saveTimer);
  saveTimer = setTimeout(flushCanvasSave, delay);
}
...
async function flushCanvasSave() {
  ...
  if (saveInFlight || !pendingSaveBody) return;

  const body = pendingSaveBody;
  ...
  try {
    ...
    console.error("Canvas Hand 保存失败", error);
    if (!pendingSaveBody) pendingSaveBody = body;
    saveTimer = setTimeout(flushCanvasSave, 2000);
  } finally {
    saveInFlight = false;
    if (pendingSaveBody && !saveTimer) saveTimer = setTimeout(flushCanvasSave, 0);
  }
}
...
async function onCanvasMount(editor) {
  ...
  if (snapshot && snapshot.document) {
    ...
    queueCanvasSave(getSnapshot(editor.store), 0); // 带 schema 的合法空档
